...
The recommended way to install `grove-context` is by using the `grove` meta-CLI, which manages all tools within the Grove ecosystem.
...
Once the `grove` CLI is set up, you can install `grove-context` with a single command:
