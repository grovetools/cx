	...
	return &harness.Scenario{
		...
		Steps: []harness.Step{
			harness.NewStep("Clean up existing tmux sessions", func(ctx *harness.Context) error {
				...
				fmt.Println("   🧹 Session cleanup complete")
				return nil
			}),
			...
			harness.NewStep("Launch 'cx view' in tmux", func(ctx *harness.Context) error {
				...
				fmt.Println("   📺 TUI session started (use -d flag to debug interactively)")
				...
				if err := session.StartRecording(recordingPath); err != nil {
					fmt.Printf("   ⚠️  Could not start recording: %v\n", err)
				} else {
					fmt.Printf("   🎬 Session recording started: %s\n", recordingPath)
				}
			...
			harness.NewStep("Capture final state", func(ctx *harness.Context) error {
				...
				if err != nil {
					fmt.Printf("   ⚠️  Could not capture final state: %v\n", err)
					return nil
				}
				...
				if err := session.TakeScreenshot(screenshotPath); err != nil {
					fmt.Printf("   ⚠️  Could not save screenshot: %v\n", err)
				} else {
					fmt.Printf("   📸 Final screenshot saved to: %s\n", screenshotPath)
				}
				
				// Stop recording and save
				if err := session.StopRecording(); err != nil {
					fmt.Printf("   ⚠️  Could not stop recording: %v\n", err)
				} else {
					if err := session.SaveRecording(); err != nil {
						fmt.Printf("   ⚠️  Could not save recording: %v\n", err)
					} else {
						fmt.Println("   💾 Session recording saved")
					}
				}
				
				// Print key history summary
				keyHistory := session.GetKeyHistory()
				fmt.Printf("   ⌨️  Total keys sent: %d\n", len(keyHistory))
				if len(keyHistory) > 0 {
				...
				if err := os.WriteFile(capturePath, []byte(finalCapture), 0644); err != nil {
					fmt.Printf("   ⚠️  Could not save capture: %v\n", err)
				} else {
					fmt.Printf("   📄 Final capture saved to: %s\n", capturePath)
				}
				
				fmt.Println("   📊 Test completed with enhanced navigation APIs")
